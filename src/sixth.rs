...
impl<T> LinkedList<T> {
    ...
    pub fn push_front(&mut self, elem: T) {
        unsafe {
            ...
            if let Some(old) = self.front {
                ...
                // If there's no front, then we are the empty list and need to set the back too.
    ...
    pub fn pop_front(&mut self) -> Option<T> {
        unsafe {
            ...
            self.front.map(|node| {
                // Bring Box back to life so we can move out its value and Drop it.
                let boxed_node = Box::from_raw(node.as_ptr());
                let result = boxed_node.elem;

                // Make next node into the new front.
                self.front = boxed_node.back;
                if let Some(new) = self.front {
                    // Clean up its reference to the removed node.
                    (*new.as_ptr()).front = None;
                } else {
                    // If the front is now null, then list is now empty.
                    self.back = None;
                }
...
/// ```compile_fail
/// use too_many_linked_lists::sixth::IterMut;
///
...
pub struct CursorMut<'a, T> {
    ...
    // Optional index because we have a ghost element.
    index: Option<usize>,
}
            ...
            unsafe {
                ...
                if self.cur.is_some() {
                    *self.index.as_mut().unwrap() += 1;
                    ...
                    // We just walked to the ghost element, no more index.
                    self.index = None;
                }
            }
        } else if !self.list.is_empty() {
            // We are at the ghost element, and there is a real front so we move to it.
            self.cur = self.list.front;
            self.index = Some(0);
        } else {
            // We are at the ghost element, but that's the only one so we just do nothing
        }
    }
